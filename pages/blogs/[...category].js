...
const CategoryPage = ({ categoryParam, data, metas, setMetas }) => {
  ...
  // Extract category name from slug and set metas
  useEffect(() => {
    if (actualCategorySlug) {
      const categoryName = Array.isArray(actualCategorySlug)
      ...
      setSelectedCategoryName(formattedName);
      setSelectedCategories([formattedName]);

      // Update metas
      if (typeof setMetas === "function") {
        const newMetas = {
          title: `${formattedName} | Blog Category`,
          metaTitle: `${formattedName} | Blog Category`,
          metaDescription: `Explore all articles in ${formattedName} category`,
          metaKeyword: `${formattedName}, blog, articles, insights`,
        };
        setMetas(newMetas);
      }
    }
  }, [actualCategorySlug, setMetas]);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            ...
            // Auto-select the current category from URL
            if (actualCategorySlug) {
              ...
              if (categoryFromSlug) {
                ...
                // Update metas with exact category name from API
                if (typeof setMetas === "function") {
                  const newMetas = {
                    title: `${categoryFromSlug.name} | Blog Category`,
                    metaTitle: `${categoryFromSlug.name} | Blog Category`,
                    metaDescription: `Explore all articles in ${categoryFromSlug.name} category`,
                    metaKeyword: `${categoryFromSlug.name}, blog, articles, insights`,
                  };
                  setMetas(newMetas);
                }
              }
...
CategoryPage.getInitialProps = async ({ query, req }) => {
  ...
  // Extract actual category from query
  let actualCategory = Array.isArray(category) ? category[0] : category;

  // Format category name
  let pageTitle = "Blogs | Our Articles & Insights";
  let pageDescription =
    "Explore our collection of insightful articles, tips, and guides on various topics.";
  let pageKeywords = "blog, articles, insights, guides, tips";

  if (actualCategory && actualCategory !== "[...category]") {
    const categoryName = actualCategory
      .split("-")
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
      .join(" ");

    pageTitle = `${categoryName} | Blog Category`;
    pageDescription = `Explore all articles in ${categoryName} category`;
    pageKeywords = `${categoryName}, blog, articles, insights`;
  } else {
    actualCategory = null;
  }
  ...
  initialMetas = {
    ...
    metaKeyword: pageKeywords,
  };

  return {
    categoryParam: actualCategory,
    themeData,
    initialMetas,
  };
};
