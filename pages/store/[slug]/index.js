...
export default function Storedetail({ data, setMetas, metas }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${APP_URL}api/single-store/${slug}?key=${APP_KEY}`)
      .then((res) => res.json())
      .then((json) => {
        setsinglestore(json);
        setMetas({
          ...
          title: `${
            json?.data?.store?.name
              ? json?.data?.store?.name + " Coupons & Promo Codes"
              : "Coupon & Promo Codes"
          }`,
          metaTitle: json?.data?.store?.seo_title,
          metaDescription: json?.data?.store?.seo_description,
          metaKeyword: json?.data?.store?.meta_key,
        });
        setError(null);
        setLoading(false);
